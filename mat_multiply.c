...
void fill_matrix_from_user_input(int** matrix, int row_dim, int col_dim) {
    ...
    @modifies: matrix
    */
    for (int rows = 0; rows < row_dim; rows++) {
        for (int cols = 0; cols < col_dim; cols++) {
...
int** matrix_multiply(int** matrix_a, int num_rows_a, int num_cols_a,
    ...
    *out_num_cols_c = num_cols_b;
    ...
    for (int rows_c = 0; rows_c < *out_num_rows_c; rows_c++) {
        for (int cols_c = 0; cols_c < *out_num_cols_c; cols_c++) {
            matrix_c[rows_c][cols_c] = 0;
            for (int a = 0; a < num_cols_a; a++) {
                matrix_c[rows_c][cols_c] += matrix_a[rows_c][a] * matrix_b[a][cols_c];
            }
        }
    }
    return matrix_c;
